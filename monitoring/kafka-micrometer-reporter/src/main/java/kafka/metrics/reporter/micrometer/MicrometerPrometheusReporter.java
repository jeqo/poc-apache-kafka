...
public class MicrometerPrometheusReporter implements MetricsReporter, ClusterResourceListener, KafkaMetricsReporter {
  ...
  public void configure(Map<String, ?> configs) {
    ...
    server =
      Server.builder().service("/metrics", (ctx, req) -> HttpResponse.of(registry.scrape())).http(metricsPort).build();
    server.start();
  }
  ...
  private Id meterId(Metric metric) {
    ...
    final Type type;
    if (name.endsWith("total") || name.endsWith("count")) type = Type.COUNTER; else type = Type.GAUGE;
  ...
  private void bindMeter(Metric metric) {
    ...
    try {
      ...
      if (message != null && message.contains("Prometheus requires")) LOG.info(
        "Failed to bind meter: " +
        id.getName() +
        " " +
        id.getTags() +
        ". However, this could happen and might be restored in the next refresh."
      ); else LOG.warn("Failed to bind meter: " + id.getName() + " " + id.getTags() + ".", ex);
    }
  }

  private void registerGauge(Id id, Metric metric) {
    Gauge
      .builder(id.getName(), metric, toMetricValue())
      .tags(id.getTags())
